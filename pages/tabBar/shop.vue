<template>
  <view class="page flex-col">
    <nav-bar :hasBack="true" title="商城" :fixed="true" :isShow="true"></nav-bar>
    <view class="flex-row siderbar-container">
      <view class="sider-bar">
        <view class="sider-bar-item" v-for="(item, index) in navLeft" :key="index">
          <view class="flex-row sider-bar-mar" :class="{ active: isActive === index }" @click="checked(index)">
            <view :class="{ line: isActive === index }"></view>
            <text class="ml-20 sider-bar-text" :class="{ active: isActive === index }">{{ item.text }}</text>
          </view>
        </view>
      </view>
      <view class="flex-col flex-1 right-panel">
        <view class="flex-col justify-start swiper-wrap">
          <swiper class="right-swiper" style="height: 154rpx; width: 480rpx" autoplay="true" @change="swiperChange">
            <swiper-item v-for="(item, index) in Swiperitems" :key="index">
              <image class="swiper-image" src="/static/mock/shop_swiper_image.png" mode="aspectFill" />
            </swiper-item>
          </swiper>
        ...
        <scroll-view class="scroll-view_H" scroll-x="true">
          <view class="flex-row tabs-wrap">
            <view class="u-page__tag-item" v-for="(item, index) in radios" :key="index">
              <u-tag :text="item.value" :plain="!item.checked" :name="index" @click="radioClick"> </u-tag>
            </view>
          </view>
        </scroll-view>
        <view class="flex-col list-wrap mt-16">
          ...
          >
            <image class="shrink-0 shop-image" src="/static/mock/shop_image.png" mode="aspectFill" />
            <view class="ml-12 flex-col shop-con">
              <text class="title">衣架防滑无痕围巾架丝</text>
              <view class="flex-row tag items-center">
                <view class="tag-wrap items-center flex-row" v-for="(item, index) in prodTags" :key="index">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        prodTags: ['新品上市', '好评推荐'],
        items: [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
        isActive: 0,
        navLeft: [
          { index: 0, text: '卧室场景' },
          { index: 1, text: '厨房场景' },
