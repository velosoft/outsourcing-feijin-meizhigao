  ...
  <view class="flex-row list-item mt-8">
    ...
    <view class="flex-col flex-1 right ml-10">
      <view class="flex-col title-wrapper">
        <text class="self-stretch product-title line-clamp-two">{{ item.title }}</text>
        <view class="flex-row self-stretch mt-4 tag-wraper">
          <view class="flex-col tag" v-for="(tag, index) in item.tags || []" :key="index">
      ...
      <view class="flex-col">
        ...
        <view class="flex-row items-center line-progress" v-if="activityStatus == 1">
          <u-line-progress
            ...
            style="width: 180rpx"
          ></u-line-progress>
          <text class="remainder-font numder-ramainder ml-4">仅剩{{ item.invertory || 0 }}份</text>
        </view>
        <view
        ...
        >
          <view class="flex-col items-center btn-bg-wraper btn-bg-pos">
            <image class="btn-bg" src="/static/images/btn_bg_01.png" v-if="activityStatus == 0" />
            <image class="btn-bg" src="/static/images/btn_bg_02.png" v-if="activityStatus == 1" />
          </view>
    ...
    computed: {
      detailUrl() {
        switch (this.item.type) {
          case 'product':
            return `/pages/detailPages/productDetail?id=${this.item.id || ''}&from=activity`;
          case 'service':
            return `/pages/detailPages/serviceDetail?id=${this.item.id || ''}&from=activity`;
          case 'course':
            return `/pages/detailPages/courseDetail?id=${this.item.id || ''}&from=activity`;
          default:
            return '';
        }
      },
