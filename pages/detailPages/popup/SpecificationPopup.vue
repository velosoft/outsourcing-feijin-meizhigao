<template>
  <view class="flex-col container">
    <img
      class="icon-close"
      src="/static/images/icon_popup_close.png"
      @click="onClose"
    />
    <view class="flex-col">
          ...
          <view class="flex-col">
            <view class="flex-row items-center self-stretch">
              ...
              <view v-else class="normal-color">
                <text class="price-symbol">￥</text>
                <text class="price"
                  >{{ spec.priceRange[0].toFixed(2) }}-{{
                    spec.priceRange[1].toFixed(2)
                  }}</text
                >
              </view>
            </view>
            <text class="self-start com-font stock-text mt-16"
              >库存：{{ spec.invertory }}件</text
            >
          </view>
      ...
      <view
        class="flex-col mt-30"
        v-for="(item, idx) of spec.specialSpecs"
        :key="idx"
      >
        <text class="self-start spec-text">{{ item.title }}</text>
        <selector
...
export default {
  ...
  methods: {
    ...
    onSpecialChange(specIdx, optionIdx) {
      ...
      this.lastChoiceText =
        "已选：" + this.spec.specialSpecs[specIdx].options[optionIdx];
      this.userChoice[spec.title] = spec.options[optionIdx];
    },
    goNext() {
      this.$emit("goNext");
