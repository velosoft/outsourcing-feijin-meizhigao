<template>
  <view class="flex-col page">
      ...
      <view class="flex-col justify-start history-search" v-if="isSearch">
        <view class="flex-col history-con">
          <view class="flex-row justify-between items-center self-stretch">
            ...
            <image class="icon-delete" src="/static/images/icon_del_01.png" @click="clearHistory" />
          </view>
          <toggle-list class="equal-division mt-14" :tabsArray="tabsArray" @onClick="onClick" @onClose="onClose" />
        </view>
      </view>
      <view class="flex-col" v-else>
        <fj-sticky>
          <tabs :tabList="tabList" :tabCur="tabCur" @tabChange="onTabChange"></tabs>
        </fj-sticky>
        <list-container
    ...
    <confirm-panel :isShow="showDialog" :isnormal="false" @cancel="onCancel" @confirm="onConfirm"></confirm-panel>
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onClose(val) {
        this.tabsArray = this.tabsArray.filter((item) => item.label !== val.label);
      },
