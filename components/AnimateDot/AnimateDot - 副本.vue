<template>
  <view class="animateDotComp">
    <view class="animate-dot" v-for="(item, index) in animates" :key="index" :style="{top: item.top, left: item.left}"
    ...
    methods: {
      ...
      startAnimation(linePos) {
        ...
        this.$nextTick(() => {
          ...
          animate.animationData = animation.export()
          bezier_points.forEach((i, idx) => {
            animation.left(i.x).top(i.y).step({
              duration
            });
          });
          // animation.width(0).height(0).step({
          //   duration: 0
          // });
          animate.animationData = animation.export()
          console.log(animate);
