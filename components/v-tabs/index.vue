<template>
  <view class="vTabsComp">
    <FjSticky :bgColor="stickybg" :disabled="stickyDisabled" :zIndex="zIndex" :offsetTop="offsetTop" :customNavHeight="customNavHeight" @change="onSticky">
      <view>
        <slot name="stickyTop"></slot>
        <view class="tabs-wrap" :style="{ position: fixed ? 'fixed' : 'relative', zIndex: zIndex, padding, background: bgColor }">
          		...
          		<view class="v-tabs__container" :style="{
          		  ...
          		  height
          			...
          			<view class="v-tabs__container-item" v-for="(v, i) in tabs" :key="i" :style="{
          				...
          				fontSize: current == i && activeFontSize ? activeFontSize : fontSize,
          				fontWeight: bold || (activeBold && current == i) ? 'bold' : fontWeight,
          				justifyContent: scroll ? '' : 'center' ,
          				flex: scroll ? '' : 1,
          				padding: paddingItem
          			  }" @click="change(matchKey ? v[matchKey] : i)">
          				<image v-if="v.iconSrc" class="icon" :style="{height: iconSize}" :src="v.iconSrc" mode="heightFix"></image>
        ...
        <view v-if="fixed" class="v-tabs__placeholder" :style="{height, padding}"></view>
        <slot name="stickyBottom"></slot>
      </view>
    </FjSticky>
    <template v-if="hasSwiper">
      <swiper :current="current" :duration="200" class="swiper-container swiper-container-1" @change="handleSwiperChange" @transition="handleTransition" v-if="swiperScroll">
        <swiper-item class="swiper-item" v-for="(item, index) of tabs" :key="index">
          <scroll-view
            class="scroll-view"
            :scroll-y="true"
      ...
      <swiper
        ...
        v-else
      >
        <swiper-item class="swiper-item" :class="[`swiper-item-${index}`]" v-for="(item, index) of tabs" :key="index">
          <view class="content-container">
            <slot :name="`tab-content-${index}`"></slot>
          </view>
        </swiper-item>
      ...
      compuntedSwiperHeight(addHeight = 0) {
        ...
        setTimeout(() => {
          this.$nextTick(() => {
            ...
            query
              .select(`.swiper-container .swiper-item-${current} .content-container`)
              .boundingClientRect((data) => {
