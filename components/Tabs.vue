<template>
	<view class="tabs_com" :style="{'background': background}">
		<view class="tabs normal" v-if="tabList.length <= max && !hasScroll">
			<view class="line" :style="{width: `${100/tabList.length}%`, left: `${(100/tabList.length)*TabCur}%`}"></view>
			<view class="tab" v-for="(item, index) of tabList" :key="index" @click="tabChange(index)">
				<view class="label" :class="{active: index === TabCur}">{{item.name}}</view>
				<view class="count" v-if="item.count">{{item.count}}</view>
			</view>
		</view>
		<view class="tabs_container" v-else>
			<scroll-view scroll-anchoring scroll-with-animation scroll-x="true" :scroll-left="lineLeft-(750/pixelRatio/2)" class="tabs_scroll">
				<view class="tabs">
		...
		methods: {
			...
			computedLeft(e){
				setTimeout(() => {
					...
					query.selectAll('.tabs .tab').boundingClientRect(data => {
